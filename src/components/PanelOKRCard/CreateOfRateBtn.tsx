...
const CreateOfRateBtn = (props: Props) => {
  const [records, setRecords] = useState<ReportRecordResponse[]>([]);
  const handleDataClick = async () => {
    props.onOpenChange();
    setRecords(unpackActionResponse(await getReportsAction("AverageOProgress")));
  };
  return (
    <>
      <button className="w-3/12" onClick={handleDataClick}>
        <Card className="w-12/12 ml-1 mt-5 h-40 bg-white shadow-14">
          <CardBody className="overflow-visible p-0 ">
            <div className=" ml-3 mt-6 justify-center">
              <CircularProgress
                classNames={{
                ...
                className="ml-30"
                color="primary"
                value={props.averageOProgress}
                strokeWidth={2}
                showValueLabel={true}
              />
              <div className="ml-27 flex">
                <label className="">目標平均進度</label>
...
export default CreateOfRateBtn;
