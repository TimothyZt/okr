...
interface DropdownProps {
  id: string;
  title: string;
  indicatorSource: Record<string, string>;
  selected: number[];
  setSelected: (setSelect: number[]) => void;
  options: Option[];
  setOptions: (setSelect: Option[]) => void;
  selectString:string;
}
    ...
    <div className="relative z-50 ">
      ...
      <div>
        <select
          ...
          id={id}
        >
          {Object.entries(indicatorSource).map(([key, value], id) => (
            <option
              key={key}
              value={id + 1}
        ...
        <div className="flex flex-col items-center">
          <input name="values" type="hidden" defaultValue={""} />
                ...
                <div className="mb-2 flex rounded border border-stroke py-2 pl-3 pr-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                  <div className="flex flex-auto flex-wrap gap-3">
                    {selected.map((index) => (
                      <div
                        key={index}
                        className=" flex items-center justify-center rounded border-[.5px] border-slate-300 bg-gray px-2.5 py-0 text-sm font-medium text-black dark:border-strokedark dark:bg-white/30"
                      >
